...
public class Game {
	...
	public Game (int rows, int cols, int numRobots) {
		...
		if (numRobots > Onslaught.MAXROBOTS) {
			System.out.println("***** Trying to create Game with " + numRobots + " robots; only " + Onslaught.MAXROBOTS + " are allowed!");
			System.exit(0);
		}
		...
		//Add robots to the arena at random locations and make sure a robot doesn't spawn at the player's location
		while (numRobots > 0) {
